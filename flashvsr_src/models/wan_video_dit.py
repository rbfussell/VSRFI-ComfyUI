...
if TRITON_AVAILABLE:
    try:
        ...
        print(f"[VSRFI] Warning: Failed to load sparse_sageattn: {e}")
...
COMPATIBILITY_MODE = False  # Force PyTorch native attention
_attention_backend_logged = False  # Only log once per session

def get_attention_backend_info():
    """Returns a string describing available attention backends."""
    backends = []
    if FLASH_ATTN_3_AVAILABLE:
        backends.append("FlashAttention-3")
    if FLASH_ATTN_2_AVAILABLE:
        backends.append("FlashAttention-2")
    if SAGE_ATTN_AVAILABLE:
        backends.append("SageAttention")
    if BLOCK_ATTN_AVAILABLE:
        backends.append("BlockSparseAttention")
    if SPARSE_SAGE_AVAILABLE:
        backends.append("SparseSageAttention (Triton)")
    backends.append("PyTorch SDPA (fallback)")
    return backends

def log_attention_backend(backend_name: str, is_masked: bool = False):
    """Log which attention backend is being used (once per session)."""
    global _attention_backend_logged
    if not _attention_backend_logged:
        mask_info = " (with sparse mask)" if is_masked else ""
        print(f"[VSRFI] Using attention backend: {backend_name}{mask_info}")
        print(f"[VSRFI] Available backends: {', '.join(get_attention_backend_info())}")
        if COMPATIBILITY_MODE:
            print("[VSRFI] Compatibility mode ENABLED - using PyTorch native attention")
        if not TRITON_AVAILABLE:
            print("[VSRFI] Warning: Triton not available - sparse attention disabled")
        _attention_backend_logged = True

def set_compatibility_mode(enabled: bool):
    """Enable or disable compatibility mode (forces PyTorch native attention)."""
    global COMPATIBILITY_MODE, _attention_backend_logged
    COMPATIBILITY_MODE = enabled
    _attention_backend_logged = False  # Reset so we log the new backend

def reset_attention_logging():
    """Reset attention logging so it logs again on next call."""
    global _attention_backend_logged
...
class WindowPartition3D:
    ...
    def partition(x: torch.Tensor, win: Tuple[int, int, int]):
        B, F, H, W, C = x.shape
        wf, wh, ww = win
        assert F % wf == 0 and H % wh == 0 and W % ww == 0, "Dims must divide by window size." + str(F) + ", " + str(H) + ", " + str(W) + ", " + str(wf) + ", " + str(wh) + ", " + str(ww)
...
def flash_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, num_heads: int, compatibility_mode=False, attention_mask=None, return_KV=False):
    # Use global compatibility mode if not explicitly set
    use_compat_mode = compatibility_mode or COMPATIBILITY_MODE

    if attention_mask is not None:
        seqlen = q.shape[1]
        seqlen_kv = k.shape[1]

        # Check if we can use sparse attention, otherwise fall back to dense
        can_use_block_attn = USE_BLOCK_ATTN and BLOCK_ATTN_AVAILABLE
        can_use_sparse_sage = SPARSE_SAGE_AVAILABLE and sparse_sageattn is not None

        if use_compat_mode or (not can_use_block_attn and not can_use_sparse_sage):
            # Fallback: Convert sparse mask to dense and use PyTorch SDPA
            log_attention_backend("PyTorch SDPA (dense fallback)", is_masked=True)
            q = rearrange(q, "b s (n d) -> b n s d", n=num_heads)
            k = rearrange(k, "b s (n d) -> b n s d", n=num_heads)
            v = rearrange(v, "b s (n d) -> b n s d", n=num_heads)
            # Note: We ignore the sparse mask and use full attention
            # This is slower but works without Triton
            x = F.scaled_dot_product_attention(q, k, v)
            x = rearrange(x, "b n s d -> b s (n d)", n=num_heads)
        elif can_use_block_attn:
            log_attention_backend("BlockSparseAttention", is_masked=True)
            ...
            # Use sparse sage attention (Triton-based)
            log_attention_backend("SparseSageAttention (Triton)", is_masked=True)
            q = rearrange(q, "b s (n d) -> b n s d", n=num_heads)
            k = rearrange(k, "b s (n d) -> b n s d", n=num_heads)
            v = rearrange(v, "b s (n d) -> b n s d", n=num_heads)
            base_blockmask = attention_mask
        ...
        log_attention_backend("PyTorch SDPA (compatibility mode)")
        q = rearrange(q, "b s (n d) -> b n s d", n=num_heads)
        k = rearrange(k, "b s (n d) -> b n s d", n=num_heads)
        v = rearrange(v, "b s (n d) -> b n s d", n=num_heads)
        x = F.scaled_dot_product_attention(q, k, v)
        ...
        log_attention_backend("FlashAttention-3")
        q = rearrange(q, "b s (n d) -> b s n d", n=num_heads)
        k = rearrange(k, "b s (n d) -> b s n d", n=num_heads)
        v = rearrange(v, "b s (n d) -> b s n d", n=num_heads)
        ...
        log_attention_backend("FlashAttention-2")
        q = rearrange(q, "b s (n d) -> b s n d", n=num_heads)
        k = rearrange(k, "b s (n d) -> b s n d", n=num_heads)
        v = rearrange(v, "b s (n d) -> b s n d", n=num_heads)
        ...
        log_attention_backend("SageAttention")
        q = rearrange(q, "b s (n d) -> b n s d", n=num_heads)
        k = rearrange(k, "b s (n d) -> b n s d", n=num_heads)
        v = rearrange(v, "b s (n d) -> b n s d", n=num_heads)
        x = sageattn(q, k, v)
        ...
        log_attention_backend("PyTorch SDPA (fallback)")
        q = rearrange(q, "b s (n d) -> b n s d", n=num_heads)
        k = rearrange(k, "b s (n d) -> b n s d", n=num_heads)
        v = rearrange(v, "b s (n d) -> b n s d", n=num_heads)
        x = F.scaled_dot_product_attention(q, k, v)
